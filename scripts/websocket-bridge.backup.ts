  ...
  try {
    ...
    if (agents.length === 0) {
      ...
      for (const agent of agents) {
        const angle = Math.random() * Math.PI * 2;
        const dist = 3 + Math.random() * 8;
        // Use persisted color from DB, or assign a new one
        ...
        if (!agent.color) {
          prisma.agent.update({ where: { id: agent.id }, data: { color: botColor } })
...
// ─── Graceful Shutdown ──────────────────────────────────────────
// Clears intervals, disconnects Prisma, and closes WebSocket server on Ctrl+C
// Prevents connection pool exhaustion across dev restarts
// Added: 2026-02-19
...
function gracefulShutdown(signal: string) {
  ...
  // Disconnect Prisma to release DB connection pool
